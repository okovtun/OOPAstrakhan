...
namespace Geometry
{
	...
	public:
		...
		void draw()const
		{
			...
			//1) Создаем контекст устройства, это то, на чем мы будем рисовать:
			HDC hdc = GetDC(hwnd);
			...
			//После того, как кисть не нужна, е енужно удалить:
			DeleteObject(hBrush);
			DeleteObject(hPen);
			// После того, как мы все нарисовали, нужно освободить ресурсы, занимаемые контекстом устройства:
			ReleaseDC(hwnd, hdc);
		}
		void info()const
		{
